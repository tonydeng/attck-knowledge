...
### T1548.001 - Windows UAC绕过

**通俗理解：** 找到一个不需要点击"是"就能偷偷用管理员权限执行程序的方法。

**详细说明：**
Windows UAC在设计上允许某些系统程序自动提升权限（自动提升，auto-elevate），如`fodhelper.exe`、`cmstp.exe`、`eventvwr.exe`等。攻击者通过修改注册表将这些程序的行为重定向到执行恶意代码，从而在用户不知情的情况下以管理员权限执行恶意程序。

### T1548.003 - Sudo和Sudo缓存

**通俗理解：** 使用sudo输入一次密码后，短时间内再使用sudo就不需要再输密码了，攻击者可以利用这个时间窗口。

**详细说明：**
sudo默认会缓存凭据5分钟（可通过`timestamp_timeout`配置）。攻击者在获得标准用户shell后，如果在sudo缓存有效期内，可以直接使用`sudo`命令而不需要密码。此外，某些系统配置了`NOPASSWD`选项允许特定用户在不输入密码的情况下执行sudo。
