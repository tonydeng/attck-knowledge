...
**通俗理解：** 域名像地铁一样，今天走这个站明天换那个站。

**详细说明：**
Fast Flux 通过频繁更改 DNS A 记录，将域名解析到不断变化的IP池中。单域模式（Single-Flux）：为域名分配10-50个IP，每个IP存活60-300秒。双域模式（Double-Flux）：不仅轮换A记录，还轮换 NS 记录（DNS解析服务器），防御者更难追踪真实后端。Fast Flux 通常利用庞大的肉鸡网络作为代理节点。

### T1568.002 - 域名生成算法（DGA）

**通俗理解：** 恶意软件每天按算法"算"出一堆域名，像买彩票一样试哪个能连上。

**详细说明：**
恶意软件通过种子值（通常是当前日期、时间）输入加密算法，生成大量看似随机的域名。攻击者注册其中一小部分指向C2服务器。DGA有多个变种：基于日期的DGA（Conficker每天生成不同域名）、基于AES的DGA（LockBit）、基于单词列表的DGA（产生看似正常的英文词组）、混合DGA（结合时间戳和硬编码种子）。

### T1568.003 - DNS计算

**通俗理解：** 从正常网站的IP地址中"读"出暗号。

**详细说明：**
与DGA直接生成域名不同，DNS Calculation 通过解析正常域名的DNS响应中的数值字段（如IP地址的各个八位组、TTL值、优先级等），经过数学运算得到C2服务器的地址或端口。DNS查询本身是对完全正常域名的合法解析请求。
