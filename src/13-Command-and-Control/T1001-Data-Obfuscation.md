...
**通俗理解：** 在真情报中掺假信息，让情报看起来像没用的废话。

**详细说明：**
攻击者在C2通信中插入随机生成的填充数据以扰乱流量分析和签名检测。例如在 HTTP POST body 中混合真实数据和随机字节、在 DNS 查询域名中附加随机字符串、在 HTTP 请求头中添加自定义假字段。垃圾数据由伪随机生成器产生，使每次通信的字节流呈现不同的统计特征，绕过基于熵值分析的异常检测。

### T1001.002 - 隐写术

**通俗理解：** 把字条藏在画里——C2指令藏在图片的像素点中，肉眼看不出来。

**详细说明：**
攻击者将C2指令隐藏在看似无害的数字媒体文件中。常见载体包括：图片文件的像素最低有效位（LSB）、音频文件的频谱掩蔽、视频流的冗余数据帧、HTML/CSS/JS文件中的空白字符和注释。隐写术使C2通信在流量分析和网络取证中几乎不可见，因为载体文件在外观上完全正常。

### T1001.003 - 协议模仿

**通俗理解：** 让C2指令"穿上"合法协议的"制服"，混在正常流量中。

**详细说明：**
攻击者模仿合法应用层协议的外形和行为。C2流量使用合法协议的标准端口、报文头结构和交互模式。常见的模仿对象包括：HTTP/2和HTTP/3（gRPC、REST API）、WebSocket实时通信（聊天、协作工具）、MQTT（物联网通信）、SMTP/IMAP（邮件通信）、DNS over HTTPS（DoH）。高级攻击者还会模仿特定 SaaS 平台的 API 调用模式（如 Microsoft Graph API、Slack API）。
