...
**通俗理解：** 用常见的编码方式给C2数据"化妆"。

**详细说明：**
使用公开的标准编码算法，包括 Base64（RFC 4648）、Base64URL（URL安全的变体）、Base58（不含易混淆字符）、Base32（大写+数字）、Base16（十六进制）。标准编码的好处是任何编程语言都内置支持，计算开销小。攻击者还使用多次编码（如先Base64再Base64）或编码组合（如Hex+Base64）来规避简单的解码检测。

### T1132.002 - 非标准编码

**通俗理解：** 用自己发明的密码表编码，让别人更难破解。

**详细说明：**
攻击者自定义编码算法，如重新排列Base64字符表（将标准 `A-Za-z0-9+/=` 打乱顺序）、基于字典的替换编码（将命令映射到数字ID）、位域编码（将数据嵌入到特定格式字段的不同比特位）。非标准编码的优势在于没有公开的编码特征，威胁情报和IDS规则难以建立检测模式。
