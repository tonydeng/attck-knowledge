...
**通俗理解：** 使用厂家设定的默认密码（如 admin/password）登录设备或软件。

**详细说明：** 许多网络设备（路由器、防火墙）、数据库系统、管理控制台出厂时带有默认用户名和密码。如果管理员没有修改这些默认凭据，攻击者可以轻易登录。常见默认组合如 `admin/admin`、`root/root`、`administrator/password`。

### T1078.002 - 域账户

**通俗理解：** 使用公司网络域中的账户登录各种资源。

**详细说明：** Active Directory 域环境中的账户可以访问域内的各种资源。攻击者如果获得了一个域用户的有效凭据，就可以登录域内的多台计算机和系统。特别是域管理员账户的凭据，可以控制整个域。

### T1078.003 - 本地账户

**通俗理解：** 使用电脑本地的账户登录单台计算机。

**详细说明：** 每台 Windows/Linux/macOS 计算机都有本地用户账户。攻击者如果获得了本地管理员账户的凭据，就可以完全控制这台计算机。许多组织在多台计算机上使用相同的本地管理员密码，使得攻击者可以横向移动到更多系统。

### T1078.004 - 云账户

**通俗理解：** 使用云服务（AWS、Azure、Google Cloud）的账户登录云控制台。

**详细说明：** 随着企业上云，云账户的安全变得越来越重要。IAM 用户、服务账户、API 密钥都是攻击者的目标。一个拥有高权限（如 AWS AdministratorAccess）的云账户凭据泄露，可以让攻击者完全控制整个云环境。
