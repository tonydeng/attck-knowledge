...
```mermaid
graph TD
    A["攻击者获取云平台<br/>有效凭据"] --> B["使用AWS CLI/Azure CLI<br/>验证凭据有效性"]
    B --> C["枚举账户权限<br/>aws sts get-caller-identity"]
    C --> D["发现高权限策略<br/>如AdministratorAccess"]
    D --> E["利用有效凭据<br/>访问敏感云资源"]
    E --> F["创建后门密钥<br/>维持云环境持久访问"]
