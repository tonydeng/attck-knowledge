...
- [T1098.001 - 额外云凭据](./T1098/T1098.001-Additional-Cloud-Credentials.md) — 给云账户搞一把"备用钥匙"——虽然主钥匙可能被换，但备用钥匙仍然有效。
- [T1098.004 - SSH 授权密钥](./T1098/T1098.004-SSH-Authorized-Keys.md) — 在服务器管理员家的门垫下放一把自己的钥匙——每次来都不用敲门。
- [T1098.007 - 额外本地或域组](./T1098/T1098.007-Additional-Local-or-Domain-Groups.md) — 把自己加进公司的"核心高管群"——拿到管理员团队的成员资格。
...
**用人话说：** 账户操纵是攻击者通过修改现有账户的权限或创建新的高权限账户来获得持久化特权访问的一类技术。包括添加SSH公钥到root的authorized_keys、将恶意账户加入Domain Admins组、为云服务账户绑定高权限IAM角色、创建额外的API访问密钥等。这就像攻击者混进了公司的HR系统，悄悄把自己的职位从"实习生"改成了"系统管理员"——下次他再来的时候就是合法的高权限身份了。
