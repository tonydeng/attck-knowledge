...
| 维度 | 你需要知道的 |
|------|-------------|
| 这是什么？ | 攻击者劫持SCCM、Ansible等企业软件分发工具，批量推送恶意软件 |
| 为什么危险？ | 一次性影响数百台电脑，流量看起来像正常软件更新，难以区分 |
| 谁需要关心？ | IT管理员、SOC分析师、企业安全团队 |
| 你的第一步防御 | 监控软件部署工具的异常任务创建和执行日志 |
| 如果只做一件事 | 审计软件部署工具的权限配置，确保只有授权人员能创建部署任务 |
...
**用人话说：** 这条规则在监控SharpSCCM这类SCCM滥用工具的执行。SharpSCCM是红队常用工具，可以用来通过SCCM服务器向所有受管电脑推送恶意软件。如果你看到SharpSCCM.exe在非管理终端上运行，或者看到SCCM日志中出现异常的部署任务，那可能是攻击者在利用你的软件分发渠道进行横向移动。
