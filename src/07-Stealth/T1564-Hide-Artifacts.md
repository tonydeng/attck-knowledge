...
**通俗理解：** 给文件打上"隐藏"标签，让系统默认不显示。

**详细说明：**
在Windows上使用`attrib +h`命令或文件属性设置隐藏标记；在Linux/macOS上使用以"."开头的文件名。这是最基础也是最常用的隐藏手段。攻击者经常将恶意DLL或可执行文件存放在系统目录（如`C:\Windows\System32`）中并设置为隐藏，利用大量系统文件作为掩护。

### T1564.002 - 隐藏用户

**通俗理解：** 创建在用户列表中看不到的"幽灵账户"。

**详细说明：**
在Windows中创建以"$"结尾的本地用户账户（如`admin$`），该账户不会出现在常规的用户管理界面（如`net user`命令输出）中。攻击者使用隐藏账户作为后门，即便系统管理员定期检查用户列表也很难发现。

### T1564.003 - 隐藏窗口

**通俗理解：** 让恶意程序在后台运行，用户看不到任何窗口。

**详细说明：**
使用`CreateProcess` API的`dwCreationFlags`参数设置为`CREATE_NO_WINDOW`，或在PowerShell中使用`-WindowStyle Hidden`参数。用于隐藏后门程序、键盘记录器等的运行窗口。

### T1564.004 - NTFS文件属性

**通俗理解：** 利用NTFS文件系统的特殊属性隐藏信息。

**详细说明：**
使用`attrib +s +h`将恶意文件标记为"系统文件"和"隐藏文件"。在Windows默认配置下，受保护的系统文件不会显示在资源管理器中。攻击者常将恶意文件放在`C:\`根目录并设置系统和隐藏属性。

### T1564.005 - 隐藏文件系统

**通俗理解：** 创建操作系统不识别或者不显示的隐藏分区。

**详细说明：**
在磁盘上创建没有盘符的分区、使用TrueCrypt/VeraCrypt创建隐藏加密卷，或利用Windows的EFI系统分区来存储恶意载荷。这些分区在常规的"我的电脑"中不可见。

### T1564.006 - 运行虚拟实例

**通俗理解：** 在虚拟机里运行恶意操作，宿主机的安全软件看不到。

**详细说明：**
攻击者在受害系统上启动一个轻量级虚拟机或容器，在其中执行恶意命令。这样做的好处是：虚拟环境与宿主机隔离，宿主机的安全监控工具（如EDR）无法检测虚拟机内部的恶意活动。Ragnar Locker等勒索软件曾使用此技术来隐藏加密过程。

### T1564.007 - VBA隐藏

**通俗理解：** 把恶意代码隐藏在Office文档的宏代码中。

**详细说明：**
通过修改VBA（Visual Basic for Applications）宏代码的编译形式（如压缩或混淆源代码），使安全分析工具难以提取和检视原始的恶意宏代码。常用于钓鱼邮件附件中的恶意文档。

### T1564.008 - 邮箱隐藏规则

**通俗理解：** 在邮箱中设置自动规则，隐藏特定邮件。

**详细说明：**
攻击者登录受害者的邮箱账户后，创建邮件规则将特定邮件（如安全告警、登录通知）自动移到隐藏文件夹或标记为已读并归档。这可以掩盖攻击者对邮箱的异常访问行为。

### T1564.009 - 资源派生

**通俗理解：** 利用macOS系统的"资源分支"功能隐藏数据。

**详细说明：**
macOS HFS+文件系统支持资源派生（Resource Fork），可以将额外数据存储在文件的一个独立分支中。常规的文件操作不会显示资源派生中的内容，安全工具扫描主文件时也可能忽略这些隐藏数据。

### T1564.010 - 进程参数欺骗

**通俗理解：** 在进程列表中显示伪造的命令行参数，掩盖真正的恶意行为。

**详细说明：**
创建进程时，使用`CreateProcess` API分别指定`lpCommandLine`参数（实际执行的命令）和`ImagePathName`参数（进程列表中显示的名称）。安全工具查看进程列表时看到的是无害的合法程序名，但实际上运行的是恶意代码。

### T1564.011 - 忽略进程中断

**通俗理解：** 在进程管理工具中隐藏指定进程，让任务管理器看不到。

**详细说明：**
通过修改进程的PEB（Process Environment Block，进程环境块）结构中的`BeingDebugged`标志或挂钩进程枚举API，使进程在系统进程列表中不可见。此技术可用于隐藏恶意后门进程。

### T1564.012 - 文件/路径排除

**通俗理解：** 把恶意文件存放在杀毒软件不会扫描的排除目录中。

**详细说明：**
攻击者将恶意文件放置在杀毒软件配置中已被排除扫描的目录中。这些排除路径通常是为了提升性能或减少误报而设置的（如备份软件的工作目录）。攻击者也可以利用Windows安全中心或Microsoft Defender配置中的排除项，将其恶意文件写入这些免扫描目录。
