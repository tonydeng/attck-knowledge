...
> **注册AppInit DLL，所有进程加载user32.dll时自动加载**

## 30秒速查卡

| 维度 | 你需要知道的 |
|------|-------------|
| 这是什么？ | 注册AppInit DLL，所有进程加载user32.dll时自动加载 |
| 为什么危险？ | AppInit DLL在所有GUI进程启动时自动注入 |
| 谁需要关心？ | SOC分析师、终端安全工程师、威胁猎手 |
| 你的第一步防御 | 监控AppInit DLL的异常注册 |
| 如果只做一件事 | 检测AppInit_DLLs注册表键的异常修改 |
...
**AppInit DLL**（T1546.010）是 **事件触发执行**（T1546）的一个具体变体，属于 **隐蔽** 阶段的攻击技术。

> 📚 **打个比方**：就像在图书馆的每一本书里都夹了一张"同款书签"——只要任何程序加载了user32.dll（几乎所有Windows GUI程序都会加载），恶意DLL就会被自动注入，攻击面极广。

### 具体怎么理解？

注册AppInit DLL，所有进程加载user32.dll时自动加载

攻击者使用这种技术时，通常是在隐蔽阶段，想要达到特定的攻击目标。与父技术 T1546 相比，T1546.010 有自己独特的特点和使用场景。
...
攻击者在 **隐蔽** 阶段使用 AppInit DLL 技术，以下是典型的攻击步骤：
...
> **步骤1**：编写恶意的AppInit DLL

> **步骤2**：注册到注册表AppInit_DLLs值

> **步骤3**：user32.dll被加载时自动加载恶意DLL

> **步骤4**：几乎所有GUI程序都会受影响执行恶意代码
...
**用人话说：** 这条规则在检测AppInit_DLLs注册表键的异常修改。AppInit DLL在所有GUI进程启动时自动注入。
...
| **AppInit DLL** | T1546.010，ATT&CK框架中定义的一种具体攻击技术 |
| **事件触发执行** | T1546，AppInit DLL所属的父技术类别 |
...
- [MITRE ATT&CK - AppInit DLL (T1546.010)](https://attack.mitre.org/techniques/T1546/010/)
