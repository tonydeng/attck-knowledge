...
> **Regsvr32就是利用regsvr32.exe注册或加载远程DLL。**
...
**通俗解释：**
利用regsvr32.exe注册或加载远程DLL



> 🌐 **打个比方**：就像用浏览器下载恶意软件——攻击者利用CMSTP（连接管理器配置）从远程服务器下载和执行恶意代码。

**技术原理：**
T1218.003 是 系统二进制代理执行（T1218）的子技术，专注于Regsvr32这一特定方面。攻击者在隐蔽阶段，通过Regsvr32来获取目标系统或组织的相关信息，为后续攻击步骤做准备。
...
> **步骤2**：使用regsvr32.exe注册COM组件

> **步骤3**：利用/s参数调用scrobj.dll执行脚本

> **步骤4**：从远程URL下载Payload绕过白名单
...
- **参考链接**: [MITRE ATT&CK - T1218.003](https://attack.mitre.org/techniques/T1218/003/)
...
- [MITRE ATT&CK - T1218.003 检测](https://attack.mitre.org/techniques/T1218/003/detections/)
