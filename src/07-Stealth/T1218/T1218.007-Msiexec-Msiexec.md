...
> 📋 **打个比方**：就像用官方办公软件执行隐藏的恶意宏——攻击者利用Microsoft二进制文件（如Msiexec）绕过AppLocker的限制。

**技术原理：**
T1218.007 是 系统二进制代理执行（T1218）的子技术，专注于Msiexec这一特定方面。攻击者在隐蔽阶段，通过Msiexec来获取目标系统或组织的相关信息，为后续攻击步骤做准备。
...
> **步骤1**：准备托管在远程服务器上的恶意MSI包

> **步骤2**：使用msiexec /i http://... 安装远程MSI

> **步骤3**：msiexec从网络下载并缓存MSI包

> **步骤4**：安装过程中执行MSI包内恶意代码
