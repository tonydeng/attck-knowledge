...
> **Msiexec就是利用msiexec.exe安装恶意MSI包。**
...
**通俗解释：**
利用msiexec.exe安装恶意MSI包



> 📜 **打个比方**：就像用系统自带的记事本打开隐藏的密文——攻击者利用Mshta执行恶意HTA文件绕过应用白名单。

**技术原理：**
T1218.005 是 系统二进制代理执行（T1218）的子技术，专注于Msiexec这一特定方面。攻击者在隐蔽阶段，通过Msiexec来获取目标系统或组织的相关信息，为后续攻击步骤做准备。
...
> **步骤2**：使用msiexec.exe安装MSI包

> **步骤3**：MSI中的Custom Action包含恶意代码

> **步骤4**：安装过程自动执行自定义操作中恶意代码
