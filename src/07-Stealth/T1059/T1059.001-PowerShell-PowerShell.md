...
> **PowerShell就是利用PowerShell执行恶意脚本。**

## 30秒速查卡

| 维度 | 你需要知道的 |
|------|-------------|
| 这是什么？ | 利用Windows内置的PowerShell执行恶意脚本，是最常用的攻击子技术 |
| 为什么危险？ | PowerShell是系统自带工具，不会触发"安装未知软件"的告警，且功能极其强大 |
| 谁需要关心？ | 安全运维人员、SOC分析师、EDR工程师 |
| 你的第一步防御 | 启用PowerShell脚本块日志（事件ID 4104），监控-EncodedCommand参数 |
| 如果只做一件事 | 检查是否有PowerShell进程执行Base64编码命令或从网络下载脚本 |
...
**通俗解释：**
利用PowerShell执行恶意脚本，最常用的子技术

> 📚 **打个比方**：就像你有一把能开所有门的"万能钥匙"——PowerShell就是Windows系统的万能钥匙，攻击者用它来开门（执行命令）而不需要砸锁（上传恶意软件）。

**技术原理：**
T1059.001 是 命令和脚本解释器（T1059）的子技术，专注于PowerShell这一特定方面。攻击者在隐蔽阶段，通过PowerShell来获取目标系统或组织的相关信息，为后续攻击步骤做准备。
...
> **步骤1**：获取目标系统PowerShell执行权限

> **步骤2**：使用反射DLL加载或策略绕过启动PS

> **步骤3**：通过WebClient下载远程恶意脚本

> **步骤4**：Invoke-Expression在内存中直接执行不落地磁盘
...
**用人话说：** PowerShell攻击就像用系统自带的万能工具箱干坏事。如果你看到PowerShell命令里有一大串Base64编码的乱码，或者它在从网上下载东西，那就很可疑。正常人不会用-EncodedCommand参数。
