...
| 维度 | 你需要知道的 |
|------|-------------|
| 这是什么？ | 在操作系统内核或引导层植入恶意代码，拦截系统调用来隐藏所有恶意活动痕迹 |
| 为什么危险？ | 杀毒软件、任务管理器、文件搜索全部失效，即使重装系统也可能无法清除 |
| 谁需要关心？ | 内核安全工程师、恶意软件分析师、威胁情报分析师 |
| 你的第一步防御 | 启用安全启动（Secure Boot）和基于虚拟化的安全（VBS）保护内核完整性 |
| 如果只做一件事 | 使用chkrootkit或类似工具定期扫描系统，对比API返回结果与直接磁盘读取的差异 |
...
**用人话说：** Rootkit检测就像用两把不同的尺子量同一个东西。如果API告诉你"这个文件不存在"，但你直接读磁盘发现它存在，那就说明有人在撒谎。启用安全启动能防止Rootkit在系统启动前扎根，定期用离线扫描工具检查能发现隐藏的恶意内核模块。
