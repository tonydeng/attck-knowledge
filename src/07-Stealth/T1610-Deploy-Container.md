...
| 维度 | 你需要知道的 |
|------|-------------|
| 这是什么？ | 攻击者在容器环境中部署恶意容器，利用容器隔离隐藏挖矿、后门等恶意活动 |
| 为什么危险？ | 容器环境隔离了恶意活动，传统的主机监控难以检测，而且容器可以快速部署和销毁 |
| 谁需要关心？ | 容器安全工程师、SOC分析师、云安全工程师 |
| 你的第一步防御 | 监控容器的异常部署和运行 |
| 如果只做一件事 | 检测从异常镜像部署的容器 |
...
**用人话说：** 这条规则在监控从不受信任来源创建Docker容器的行为。正常情况下，容器创建应该来自可信的镜像仓库和管理员操作。如果检测到来自未知来源的容器创建，特别是使用了--rm标志（自动删除容器），很可能是在部署恶意容器进行挖矿或后门操作。
