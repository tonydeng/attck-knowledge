...
> **检测解读**：账号被盗用的危险在于攻击者利用了"信任传递"——你信任你的同事，所以也会信任他被劫持的邮箱发来的邮件。检测异常登录时间、设备变更和邮件转发规则是关键。如果老板的邮箱突然在凌晨3点从陌生IP登录，那就是危险信号。
>
> **避坑指南**：不要只看登录是否成功，要看登录的"方式"——同一账户在短时间内从不同国家登录、使用了从未见过的设备、设置了自动转发规则，这些都是账号被盗的征兆。
