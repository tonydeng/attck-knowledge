...
> **检测解读**：购买攻击工具意味着攻击者在走"捷径"——从暗网买现成的武器比自己造快得多。检测重点是发现已知攻击工具的特征签名和暗网交易关联的IP地址。如果网络中出现了Cobalt Strike的默认配置或Metasploit的特征流量，说明有人"买了装备"。
>
> **避坑指南**：不要只依赖签名检测，很多购买的工具会被重新打包或修改以绕过检测。关注工具的"使用行为"比"文件特征"更可靠——比如Cobalt Strike的默认心跳间隔、beacon通信模式等。
