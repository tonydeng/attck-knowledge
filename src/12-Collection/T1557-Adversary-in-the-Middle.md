...
### T1557.001 - LLMNR/NBT-NS中毒和SMB中继

**通俗理解：** 当受害者输错文件名时，攻击者在局域网里假装说"我知道这个文件在哪"，然后捕获受害者的登录密码。

**详细说明：**
当Windows系统无法解析某个主机名时，会发送LLMNR或NBT-NS广播查询。攻击者监听这些广播并响应，声称自己是目标主机。受害者随后尝试使用当前登录凭据进行NTLM认证，攻击者将捕获的NTLM哈希中继到其他服务进行认证。常用工具：Responder、Impacket。

### T1557.002 - ARP缓存中毒

**通俗理解：** 攻击者在局域网中喊"我是网关"，受害者的网络流量就被改道到了攻击者那里。

**详细说明：**
攻击者发送伪造的ARP（Address Resolution Protocol）数据包，将网关的IP地址映射到攻击者的MAC地址。受害者的所有网络流量都会被发送到攻击者的机器上，攻击者可以在转发前读取和修改数据包。

### T1557.003 - DHCP欺骗

**通俗理解：** 攻击者假装成网络的"地址分配员"，给受害者分配一个"特殊的"网络配置，让所有网络流量都经过攻击者。

**详细说明：**
攻击者部署伪造的DHCP服务器（称为Rogue DHCP Server），给客户端分配包含恶意DNS服务器地址或错误网关地址的网络配置。受害者获得网络配置后，域名解析或网络流量会被引导到攻击者控制的服务器。
