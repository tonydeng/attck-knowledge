...
| 维度 | 你需要知道的 |
|------|-------------|
| 这是什么？ | 攻击者把从各个地方偷来的文件先集中放到一个临时地方，就像搬家前把所有东西堆在客厅再统一打包。 |
| 为什么危险？ | 数据分段本身不直接造成数据泄露，但它是数据窃取的"传送带"——没有分段，攻击者就很难高效地组织和传输大量数据。分段操作留 |
| 谁需要关心？ | 数据安全团队、SOC分析师 |
| 你的第一步防御 | 异常目录中的批量文件创建 |
| 如果只做一件事 | 想象一下你从房间的各个角落收集要搬走的物品——书桌上的文件、抽屉里的资料、床底下的箱子——你会先把它 |
...
- [T1074.001 - 本地数据分段](./T1074/T1074.001-Local Data Staging-本地数据分段.md) — 在自己电脑上建一个临时文件夹，所有偷来的东西先存在这里再处理。
- [T1074.002 - 远程数据分段](./T1074/T1074.002-Remote Data Staging-远程数据分段.md) — 在内网的另一台服务器上建一个中转站，把各处的数据集中到那里。
...
> 数据分段（Data Staging）是攻击者把散落各处的"战利品"集中到一个地方再打包带走的过程。攻击者从不同位置收集到文件后（本地文档、共享文件、邮件附件），不会立即一个一个传出去——那样效率太低且容易被发现。而是先把所有文件复制到一个临时目录（如C:\Users\Public\Temp\或%TEMP%\），然后再统一压缩成一个zip包准备传输。Mimikatz的输出文件、网络嗅探的pcap包、数据库导出SQL文件等也会集中存放。检测方法：监控在临时目录大量创建文件（如一次性创建50个以上文件）、文件从不同路径被复制到同一个目录、以及目录中出现不常见格式文件的混合（.docx + .pcap + .sql + .kirbi）。
>
> **避坑指南**：忽略SMB管理共享异常访问；未启用PowerShell脚本块日志；加密检测阈值设置过高。
