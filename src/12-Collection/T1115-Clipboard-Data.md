...
| 维度 | 你需要知道的 |
|------|-------------|
| 这是什么？ | 攻击者偷偷读取你复制粘贴的内容——你刚才复制密码、钱包地址或机密信息时，已经被截获了。 |
| 为什么危险？ | 剪贴板数据收集通常用于窃取用户复制的密码（尤其是密码管理器中的密码）、加密货币钱包地址、财务信息和其他敏感信息。Clip |
| 谁需要关心？ | 数据安全团队、SOC分析师 |
| 你的第一步防御 | 异常的剪贴板API调用 |
| 如果只做一件事 | 你复制（Ctrl+C）一段文字时，它被临时保存在电脑的"剪贴板"中 |
...
> 剪贴板数据窃取是"最简单但最致命"的数据收集方式——攻击者不断读取剪贴板内容，捕获用户复制粘贴的所有信息。想象一下：你刚从密码管理器复制了管理员密码准备粘贴到远程桌面，攻击者的木马就已经把这个密码偷走了。加密货币钱包地址、API密钥、数据库连接字符串、机密文档片段——一切经过剪贴板的内容都可能被盗取。攻击者通过调用Windows的GetClipboardData API（或循环监听Clipboard内容变化），每秒采样一次剪贴板内容并回传。检测方法：监控进程频繁调用GetClipboardData或OpenClipboard API、以及非UI软件（如cmd.exe、powershell.exe）读取剪贴板内容——文本编辑器复制粘贴很正常，但一个系统进程或者PowerShell脚本不停读剪贴板就很可疑。
>
