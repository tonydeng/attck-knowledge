...
**通俗理解：** 像在键盘下放了一个迷你录音机，你敲的每个键都被录下来了。

**详细说明：**
Windows平台上最常见的实现方式是通过`SetWindowsHookEx`安装`WH_KEYBOARD_LL`低级键盘钩子，这样可以在用户态捕获所有键盘输入，无需管理员权限。Linux上通过读取`/dev/input/event*`设备文件或使用`XGrabKey`抓取X11键盘事件。macOS中使用`CGEventTap`或IOKit键盘事件。记录的键击被缓存到本地文件或通过HTTP/DNS/ICMP通道外传。

### T1056.002 - GUI输入捕获

**通俗理解：** 做一个假登录页面（钓鱼页面），你输入的所有信息都被骗子看到了。

**详细说明：**
攻击者通过伪造登录界面（Web或桌面应用）捕获用户输入。在Web环境中，攻击者使用浏览器劫持技术修改HTML表单的`action`属性或注入监听`submit`事件的JavaScript。在桌面应用中，攻击者使用窗口覆盖或注入输入字段监听器拦截表单输入。此方式特别针对银行网站、邮件服务和SaaS应用的登录表单。

### T1056.003 - Web门户捕获

**通俗理解：** 在网站的源代码里做手脚，在你提交表单时多复制一份给攻击者。

**详细说明：**
攻击者在被入侵的Web服务器上注入恶意代码，捕获用户通过Web门户提交的所有表单输入。通过在Web应用源代码中添加隐藏的JavaScript监听器或修改服务端表单处理逻辑，攻击者记录登录凭据、搜索查询、表格填写等所有提交的Web表单数据。

### T1056.004 - 凭证API钩取

**通俗理解：** 在操作系统"验证密码"的环节安插了一个窃听器。

**详细说明：**
攻击者钩取操作系统的凭据输入API来捕获通过该API提交的凭据。在Windows中，钩取`CredUIPromptForCredentials`和`CredUIPromptForWindowsCredentials` API。在Linux中钩取PAM（可插拔认证模块）库函数。攻击者通过DLL注入或API挂钩框架（如Detours、Microsoft Detours）实现API级别凭据拦截。
