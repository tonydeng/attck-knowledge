...
**通俗理解：** 翻看Outlook在你电脑上留下的邮件缓存文件。

**详细说明：**
攻击者从邮件客户端（如Outlook、Thunderbird）的本地缓存数据库中读取邮件。Outlook的`.pst`/`.ost`文件、Thunderbird的`mbox`/`msf`文件中存储了完整的邮件内容。在Windows上还可以通过MAPI（Messaging API）接口或COM自动化与Outlook交互来批量提取邮件。

### T1114.002 - 远程邮件收集

**通俗理解：** 像你用手机收邮件一样，攻击者用你的密码登录邮箱服务器下载所有邮件。

**详细说明：**
攻击者使用窃取的凭据通过标准邮件协议（IMAP、POP3、SMTP）或Exchange Web Services（EWS）远程连接邮件服务器，下载指定邮箱的邮件数据。这种方式不需要在目标系统上安装专用工具。

### T1114.003 - 邮件API收集

**通俗理解：** 通过邮件服务提供的"接口"以程序方式批量下载邮件，就像用爬虫抓取网页。

**详细说明：**
攻击者使用OAuth令牌或API密钥调用Microsoft Graph API、Gmail API或Exchange Online PowerShell等接口，程序化地读取、搜索和导出邮件。这种方式比传统邮件协议更隐蔽，API调用可以精细控制数据获取范围。

### T1114.004 - 自动邮件转发

**通俗理解：** 攻击者在你的邮箱设置里设了一个"邮件自动转寄"，新邮件直接送到攻击者手里。

**详细说明：**
攻击者在获取邮箱访问权限后，在邮件服务器端创建转发规则或收件箱规则，将包含特定关键词的邮件自动转发到外部邮箱。转发规则在服务器端执行，用户端不易察觉。
