...
### T1602.001 - 通过SMB/Windows共享收集

**通俗理解：** 通过Windows的网络邻居功能访问公司内部共享目录，下载共享中的文件。

**详细说明：**
攻击者使用标准的Windows SMB协议访问网络共享。在内网中获得域用户凭据后，使用`net use`命令映射网络驱动器，然后使用文件操作命令从共享驱动器中读取和复制文件。常用命令包括`dir \\server\share`列出内容，`copy \\server\share\file .`复制文件。

### T1602.002 - 通过网络设备管理收集

**通俗理解：** 攻击者登录了公司路由器或交换机的管理界面，从这些网络设备中下载配置文件和日志。

**详细说明：**
攻击者通过默认凭据或窃取的管理员密码登录网络设备（路由器、交换机、防火墙），通过TFTP、SCP或设备的文件管理功能下载配置文件、日志文件和固件。这些文件中可能包含网络拓扑信息、VPN凭据和明文密码。
