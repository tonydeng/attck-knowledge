...
| 维度 | 你需要知道的 |
|------|-------------|
| 这是什么？ | 端点拒绝服务（T1499）是攻击者用来破坏目标系统或数据的技术 |
...
- [T1499.001 - 操作系统资源耗尽](./T1499/T1499.001-OS-Resource-Exhaustion.md) — 让操作系统忙不过来了——进程太多、内存不足、文件句柄用光
- [T1499.002 - 服务资源耗尽](./T1499/T1499.002-Service-Resource-Exhaustion.md) — 针对某个服务（比如网站服务器）的连接池或线程池耗尽
- [T1499.003 - 应用层资源耗尽](./T1499/T1499.003-Application-Layer-Exhaustion.md) — 利用复杂的业务逻辑操作消耗应用的计算能力
- [T1499.004 - 应用或系统漏洞利用](./T1499/T1499.004-Application-or-System-Exploitation.md) — 利用软件本身的bug让程序崩溃
...
**用人话说：** 这条规则在检测端点拒绝服务攻击。跟网络DDoS堵带宽不同，端点DoS是直接让目标服务器"累趴下"——通过慢速连接耗尽连接池（Slowloris）、发送复杂查询占满CPU/内存、或者利用协议漏洞（如HTTP/2 Rapid Reset）让服务器忙于处理无效请求。检测的关键信号是：服务器半开连接数（SYN_RECV状态）远超基线、P99响应延迟从毫秒级飙升到秒级、CPU持续100%但业务流量并未同比例增加。端点DoS不需要海量带宽，一台笔记本就能瘫痪中等配置的服务器。
