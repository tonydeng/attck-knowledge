...
| 维度 | 你需要知道的 |
|------|-------------|
| 这是什么？ | 阻止系统恢复（T1490）是攻击者用来破坏目标系统或数据的技术 |
...
**用人话说：** 这条规则在检测攻击者删除系统恢复功能的行为。Windows的卷影副本（VSS）是勒索软件受害者最重要的"后悔药"，所以攻击者加密前第一件事就是删除所有卷影副本。检测的关键信号是：vssadmin delete shadows /all命令被执行（几乎100%的勒索攻击都包含此操作）、wbadmin delete catalog删除备份分类、bcdedit /set recoveryenabled No禁用系统恢复环境、或者wevtutil cl清除事件日志。如果你同时看到卷影删除+批量文件加密，几乎可以确定是勒索软件攻击。正常IT运维也可能删除旧卷影副本，但通常不会配合加密行为同时出现。
