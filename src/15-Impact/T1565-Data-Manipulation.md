...
**通俗理解：** 改掉硬盘上或数据库里存着的数据

**详细说明：**
攻击者修改存储在磁盘、数据库或其他持久化介质中的数据。典型场景包括：篡改数据库中的金融记录（修改账户余额、交易记录）、修改配置文件内容（改变系统行为）、篡改日志文件（掩盖入侵痕迹）。这种篡改通常需要数据库或文件系统的写入权限。

### T1565.002 - 传输数据篡改

**通俗理解：** 数据在网络上传输时被拦下来改掉再发出去

**详细说明：**
攻击者拦截并修改网络传输中的数据。典型场景包括：中间人攻击（MITM）修改应用层协议内容、篡改API响应数据、DNS欺骗将用户引导到恶意站点、修改网页中的内容。VPNFilter恶意软件在受感染路由器上执行中间人攻击，在用户访问网页时插入恶意JavaScript。

### T1565.003 - 实时系统数据篡改

**通俗理解：** 修改工业控制系统或医疗设备的运行数据，非常危险

**详细说明：**
攻击者修改实时操作系统（RTOS）或嵌入式系统中的数据。主要针对SCADA（工业控制系统）、ICS（工业控制系统）、医疗设备等实时控制系统。经典案例是Stuxnet——修改离心机转速数据的同时回传伪造的正常数据，掩盖离心机损毁。
