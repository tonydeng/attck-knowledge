...
**通俗理解：** 用你的电脑CPU/GPU帮攻击者挖矿

**详细说明：**
最常见的资源劫持形式。攻击者在受害系统上部署加密货币挖矿软件（如XMRig），利用CPU/GPU资源挖掘Monero（门罗币）等注重隐私的加密货币。在云环境中，攻击者可能利用被攻陷的云账户启动大量GPU实例进行AI模型训练或密码破解。在Web端，攻击者通过植入JavaScript挖矿脚本（如Coinhive），访问网站的用户浏览器也会参与挖矿。

### T1654.002 - 带宽资源劫持

**通俗理解：** 用你的网络带宽帮攻击者发动攻击

**详细说明：**
攻击者劫持被攻陷系统的网络带宽用于流量转发、代理服务或DDoS攻击。被感染的设备成为僵尸网络的一部分，接受C2服务器的指令向DDoS目标发送流量。在2025年，带宽资源劫持仍然是DDoS攻击的主要流量来源，Mirai等僵尸网络持续活跃，利用IoT设备的带宽发动大规模攻击。
