...
| 维度 | 你需要知道的 |
|------|-------------|
| 这是什么？ | 停止服务（T1489）是攻击者用来破坏目标系统或数据的技术 |
...
**用人话说：** 这条规则在检测攻击者故意停止关键服务的行为。攻击者在执行勒索加密或数据销毁前，通常会先停掉安全防护服务（防病毒/EDR）防止被拦截，再停掉数据库和邮件服务以释放被锁定的文件，还会停掉备份服务让后续的加密操作不被中断。检测的关键信号是：多个核心服务在同一分钟内被net stop或sc stop命令停止、防病毒软件（如WinDefend、MsMpEng）的服务突然停止、或者数据库服务在非维护窗口期被关闭。正常运维停服务会走变更流程且有窗口期，攻击者停服务往往是一口气停掉十几个关键服务。
