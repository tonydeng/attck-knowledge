...
| 维度 | 你需要知道的 |
|------|-------------|
| 这是什么？ | 账户访问移除（T1531）是攻击者用来破坏目标系统或数据的技术 |
...
**用人话说：** 这条规则在检测攻击者删除或禁用用户账户的行为。攻击者获取域管理员权限后，会用dsrm或Remove-ADUser批量删除域用户、用net user administrator 随机密码修改管理员密码让IT团队登不进去、或禁用所有Domain Admins组成员。检测的关键信号是：短时间内大量账户被删除/禁用（Event ID 4726/4725）、管理员密码被非正常重置（Event ID 4724）、或者Domain Admins组成员被批量修改。这种操作通常不是独立攻击——而是配合勒索软件或擦除器使用，攻击者删完账号再加密数据，让受害者即使想付赎金都找不到入口恢复。
