...
**通俗理解：** 攻击者改的不是对外的网站，而是公司内部的网站，比如内部公告栏、HR系统页面

**详细说明：**
攻击者首先获得内网访问权限（通常通过横向移动），然后修改内部Web服务器上的页面。内部篡改可能用于：散布虚假的公司公告（如"公司倒闭了"）、篡改内部系统的登录页面进行钓鱼、或替换内部Wiki内容传播恶意信息。内部篡改虽然不如外部篡改"上新闻"，但破坏力可能更大——员工信任内部信息，更容易上当。

### T1491.002 - 外部网页篡改

**通俗理解：** 攻击者改掉公司对外的官方网站，让所有访问者都能看到被篡改的内容

**详细说明：**
这是更常见的篡改形式。攻击者通过Web应用漏洞、CMS漏洞或服务器入侵获取Web服务器控制权，然后替换首页内容。攻击者可能使用SQL注入获取管理员凭据、利用未修补的CMS漏洞（如WordPress、Drupal）、或通过文件上传漏洞直接上传篡改页面。外部篡改的影响立竿见影——客户、合作伙伴和媒体都会看到。
