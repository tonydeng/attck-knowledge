...
| 维度 | 你需要知道的 |
|------|-------------|
| 这是什么？ | 系统关机重启（T1529）是攻击者用来破坏目标系统或数据的技术 |
...
**用人话说：** 这条规则在检测攻击者远程控制系统关机或重启的行为。攻击者通过RDP、PsExec或后门执行shutdown命令，可能立即关机中断业务（shutdown /s /t 0），也可能设置定时关机制造混乱（shutdown /s /t 60让用户看到倒计时恐慌），或者用重启触发MBR破坏载荷。检测的关键信号是：非运维人员的shutdown.exe执行记录（Event ID 1074）、短时间内多台服务器同时关机（批量远程关机）、或者关机原因代码异常（不是计划内维护）。在勒索攻击中，关机常被用作加密完成后的"最后一击"——让系统重启后无法正常启动。
