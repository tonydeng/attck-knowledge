...
| 维度 | 你需要知道的 |
|------|-------------|
| 这是什么？ | 攻击者使用 `wevtutil qe Security`、`Get-WinEvent`、`cat /var/log/auth.log` 读取系统/应用日志，分析管理员活动模式、安全告警记录、登录事件等 |
| 为什么危险？ | 攻击者通过日志分析可以判断：安全团队是否已发现入侵（检查告警日志）、管理员何时在线（选择最佳攻击窗口）、哪些账户有高权限（横向移动目标） |
| 谁需要关心？ | SOC分析师、日志管理员、安全运营团队、任何需要检测日志异常访问的安全人员 |
| 你的第一步防御 | 监控 `wevtutil qe Security` 的批量执行；监控 `Get-WinEvent` 对 PowerShell 操作日志的异常查询；配置日志完整性监控防篡改 |
| 如果只做一件事 | 对非安全运维人员执行 `wevtutil` 查询安全日志的行为立即告警——正常业务人员不需要查看安全事件日志 |
...
**用人话说：** 这条规则在监控有人用 `wevtutil` 查询安全日志。攻击者为什么要看你的安全日志？因为他们想知道自己有没有被发现——如果日志里有 EDR 告警、杀毒软件扫描记录，说明安全团队可能已经在调查了，攻击者就会加快速度或换一种攻击方式。同时，攻击者还能从日志里发现管理员的登录时间规律，选择深夜无人值守时进行数据窃取。正常情况下，只有 SOC 分析师在做事件调查时才会查安全日志。如果你看到有人在非工作时间批量查询安全日志，或者查询后紧接着清空日志，那就是攻击者在"反侦察"。
