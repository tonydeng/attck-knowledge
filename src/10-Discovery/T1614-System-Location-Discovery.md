...
| 维度 | 你需要知道的 |
|------|-------------|
| 这是什么？ | 攻击者使用 `Get-WinSystemLocale`、`Get-Culture`、`GetSystemDefaultLangID` API 获取系统语言、键盘布局、时区等信息，判断目标电脑所在的国家/地区 |
| 为什么危险？ | 定向攻击（如 Lazarus 针对韩国、APT38 针对孟加拉）用位置信息过滤非目标系统——检测到非目标区域就隐藏真实功能或直接退出，逃避安全分析师的沙箱分析 |
| 谁需要关心？ | SOC分析师、蜜罐运营团队、恶意软件分析师、威胁狩猎团队 |
| 你的第一步防御 | 监控 `Get-WinSystemLocale`、`Get-Culture`、`GetSystemDefaultLangID` 的非管理员调用；监控注册表 `Keyboard Layout\Preload` 的异常读取 |
| 如果只做一件事 | 对新进程启动后立即查询系统语言/时区并紧接着长时间 Sleep 的行为告警——这是恶意软件"判断自己是否在正确目标上"的典型特征 |
...
- [T1614.001 - 系统语言发现](./T1614/T1614.001-System-Language-Discovery.md) — 通过电脑的语言设置判断它在哪个国家。
...
**用人话说：** 这条规则在监控有人用 PowerShell 查询系统语言信息。恶意软件为什么要查你的系统语言？因为很多 APT 攻击是"定向"的——比如 Lazarus 只想攻击韩国的加密货币交易所，如果发现系统语言是中文或英文就会停止运行，避免在非目标国家暴露自己。正常情况下，只有 IT 运维在做国际化部署时才会查这个。如果你看到一个新进程启动后第一件事就是查 Get-WinSystemLocale 或 Get-Culture，那就是恶意软件在"确认自己是否在正确的目标国家"。
