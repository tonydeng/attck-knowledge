...
| 维度 | 你需要知道的 |
|------|-------------|
| 这是什么？ | 攻击者使用 nmap、Masscan 等工具扫描目标 IP 的端口开放情况和服务版本，识别 SSH、RDP、SMB、数据库等服务 |
| 为什么危险？ | 端口扫描是横向移动的前置步骤，攻击者通过发现开放端口和漏洞版本来选择攻击目标（如永恒之蓝、RDP 弱密码） |
| 谁需要关心？ | 网络安全团队、SOC分析师、防火墙管理员、任何需要检测内网扫描行为的安全人员 |
| 你的第一步防御 | 配置 IDS/IPS 检测端口扫描行为（同一源 IP 短时间内对多个目标的 SYN 包），监控 nmap/masscan 等工具的执行 |
| 如果只做一件事 | 对内网中非管理主机发起的端口扫描立即告警，因为正常业务不需要扫描其他主机的端口 |
...
**用人话说：** 这条规则在监控有人执行 nmap 端口扫描工具。nmap 是合法的网络诊断工具，网络管理员用它检查服务状态。但攻击者用它来做完全不同的事情——扫描内网中哪些主机开了哪些端口，寻找可以入侵的目标。关键判断标准是：谁在扫？扫谁？如果网络管理员在授权的维护窗口扫描自己管理的网段，那是正常操作；但如果一台普通工作站突然开始扫描整个内网的 445、3389 端口，或者凌晨有人在批量扫描服务器端口，那就是攻击者在"踩点"，为后续的横向移动做准备。
