...
| 维度 | 你需要知道的 |
|------|-------------|
| 这是什么？ | 攻击者执行 `sc query`、`net start` 或 WMI 查询 `Win32_Service` 枚举系统上所有运行的服务，识别安全产品、数据库、备份服务等关键目标 |
| 为什么危险？ | 服务发现是攻击者判断环境价值的关键步骤，通过服务名称能识别安全产品（选规避策略）、数据库服务器（凭证窃取目标）、备份系统（优先加密目标） |
| 谁需要关心？ | SOC分析师、Windows管理员、云安全团队、任何需要检测横向移动前期活动的安全人员 |
| 你的第一步防御 | 监控 `sc.exe`、`Get-Service`、WMI `Win32_Service` 查询的异常执行，建立正常IT运维的服务查询频率基线 |
| 如果只做一件事 | 对非IT人员或非运维时段出现的服务枚举命令立即告警，因为正常办公用户不需要查看后台服务列表 |
...
**用人话说：** 这条规则在监控有人用 `sc.exe` 命令查询系统服务列表。攻击者入侵后第一件事就是看看这台电脑上跑了什么服务，有没有杀毒软件、数据库、备份系统。关键是判断"谁在查"：如果是IT运维在管理服务器，那是正常操作；但如果一个普通财务人员的电脑上突然有人执行了 `sc query`，或者凌晨3点有人在批量查询服务，那就是攻击者在"踩点"，为后续的横向移动和加密做准备。
