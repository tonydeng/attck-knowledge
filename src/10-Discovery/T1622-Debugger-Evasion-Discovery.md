...
| 维度 | 你需要知道的 |
|------|-------------|
| 这是什么？ | 恶意软件调用 `IsDebuggerPresent`、`CheckRemoteDebuggerPresent`、`NtQueryInformationProcess` 检查调试器；用 `rdtsc` 指令测量执行时间检测单步调试；扫描 x64dbg、WinDbg 等调试器进程 |
| 为什么危险？ | 调试器是安全分析师逆向恶意软件的核心工具——检测到调试器后恶意软件会停止运行、执行诱饵代码或延迟数小时才激活，使动态分析完全失效 |
| 谁需要关心？ | 恶意软件分析师、沙箱运维团队、威胁狩猎团队、EDR运维 |
| 你的第一步防御 | 监控进程对 `IsDebuggerPresent`、`NtQueryInformationProcess` 的调用；监控 `rdtsc` 指令的大量执行；使用 ETW 监控异常 API 调用序列 |
| 如果只做一件事 | 对非开发工具进程调用调试检测 API 组合（IsDebuggerPresent + CheckRemoteDebuggerPresent）立即告警——这是恶意软件反分析的典型特征 |
...
**用人话说：** 这条规则在监控有进程调用调试检测 API。恶意软件为什么要检查有没有调试器？因为安全分析师用 x64dbg、WinDbg 等调试器来"解剖"恶意软件——一步一步看它怎么运行、加密密钥藏在哪、C2 服务器地址是什么。恶意软件检测到调试器后会"装死"：返回假数据、执行无害代码路径、或者延迟几小时才激活真正功能。`IsDebuggerPresent` 是最常用的检测 API，返回 true 就说明有人在调试。正常情况下，只有开发者在调试自己的程序时才会频繁调用这些 API。
