...
| 维度 | 你需要知道的 |
|------|-------------|
| 这是什么？ | 攻击者使用 `ipconfig /all`、`route print`、`arp -a`、`nslookup` 查看 IP 配置、路由表、ARP 缓存、DNS 服务器，了解系统在网络中的位置和可达范围 |
| 为什么危险？ | 网络配置是攻击者的"第一张地图"——IP 地址告诉你在哪个子网、路由表告诉你能到达哪些网段、DNS 服务器告诉你域控在哪、ARP 缓存告诉你最近和谁通信过 |
| 谁需要关心？ | SOC分析师、网络运维、蓝队威胁狩猎、任何需要检测网络侦察行为的安全人员 |
| 你的第一步防御 | 监控同一进程链中连续执行 `ipconfig` + `route` + `arp` + `nslookup` 的行为；关注非浏览器进程对外部可达性测试端点的 HTTP 请求 |
| 如果只做一件事 | 对新启动的进程在短时间内依次执行 `ipconfig /all` → `route print` → `arp -a` → `nslookup` 立即告警——这是攻击者在"画网络地图"的典型链式行为 |
...
**用人话说：** 这条规则在监控有人用 `ipconfig /all` 查看完整网络配置。攻击者为什么要查这个？因为 `ipconfig /all` 一次性返回了大量有价值的信息：本机 IP 地址（在哪个子网）、DNS 服务器地址（域控可能就是 DNS 服务器）、默认网关（网络出口）、DNS 后缀（域名信息）、DHCP 配置等。攻击者拿到这些信息后就能判断：能不能出站到互联网（C2 通信是否可行）、内部网络有几个子网（横向移动范围）、域控的 IP 地址（下一个目标）。正常情况下，用户在排查网络问题时可能偶尔用这个命令，但新启动的进程立即执行就很可疑。
