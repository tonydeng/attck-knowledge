...
| 维度 | 你需要知道的 |
|------|-------------|
| 这是什么？ | 攻击者使用 `driverquery /v`、`fltmc`、`Get-WmiObject Win32_SystemDriver` 枚举系统驱动列表，识别安全产品驱动、虚拟化驱动、有漏洞的旧版本驱动 |
| 为什么危险？ | 攻击者通过驱动发现可以：识别 EDR/AV 的内核过滤驱动（选择绕过策略）、检测虚拟化驱动（判断是否在沙箱中）、发现有 CVE 漏洞的旧版本驱动（用于 BYOD 提权） |
| 谁需要关心？ | SOC分析师、终端安全运维、恶意软件分析师、内核安全团队 |
| 你的第一步防御 | 监控 `driverquery`、`fltmc` 的异常执行；监控 `Win32_SystemDriver` 的 WMI 查询；启用驱动签名强制和 HVCI |
| 如果只做一件事 | 对非 IT 运维人员执行 `driverquery /v` 或 `fltmc` 立即告警——正常业务人员不需要查看驱动程序列表 |
...
**用人话说：** 这条规则在监控有人执行 `driverquery` 查看系统驱动列表。攻击者为什么要查驱动？有三个目的：一是找安全产品的驱动（比如 CrowdStrike 的 csagent.sys、Symantec 的 SymEFASI.sys），知道了就能绕过；二是找虚拟化驱动（vm3dmp.sys、VBoxGuest.sys），判断是否在沙箱里；三是找有已知漏洞的旧版本驱动，用 BYOD 技术加载漏洞驱动来获取内核级权限。正常情况下，只有 IT 运维在做驱动管理或故障排查时才会用这个命令。
