...
| 维度 | 你需要知道的 |
|------|-------------|
| 这是什么？ | 攻击者使用 `fsutil fsinfo drives`、`wmic logicaldisk get`、`lsusb` 枚举连接到系统的 USB 驱动器、外置硬盘、打印机等外设 |
| 为什么危险？ | 外围设备发现帮助攻击者判断是否有可移动存储（用于数据窃取或跨物理隔离网络传播）、摄像头（物理监控规避）、打印服务器（横向移动入口） |
| 谁需要关心？ | SOC分析师、终端安全运维、任何需要检测USB设备异常枚举行为的安全人员 |
| 你的第一步防御 | 监控 `fsutil fsinfo drives` 和 `wmic logicaldisk get` 的异常执行，启用USB设备安装审计日志 |
| 如果只做一件事 | 对非IT人员的电脑上执行外设枚举命令立即告警，特别是搜索可移动磁盘（DriveType=2）的行为 |
...
**用人话说：** 这条规则在监控有人用 WMI 命令查询系统连接的磁盘设备。攻击者为什么要查这个？因为他们想看看有没有插着的 U 盘或移动硬盘——如果有，就可以把窃取的数据复制到 USB 设备上带走，或者通过 USB 设备把恶意软件带入物理隔离的网络（就像震网病毒 Stuxnet 那样）。正常情况下，IT 运维人员偶尔会查询磁盘信息，但不会在命令行里特意查询 DriveType。如果你看到有人执行 `wmic logicaldisk get DriveType`，而且不是在做资产盘点或故障排查，那就值得怀疑。
