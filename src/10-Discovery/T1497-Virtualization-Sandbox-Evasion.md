...
| 维度 | 你需要知道的 |
|------|-------------|
| 这是什么？ | 恶意软件通过检测硬件特征（CPU、MAC地址）、用户行为（鼠标移动）、时间偏差（Sleep加速）来判断是否在沙箱/虚拟机中运行 |
| 为什么危险？ | 沙箱规避是高级恶意软件的核心能力，能在分析环境中隐藏真实行为，只在真实目标环境中才激活恶意功能 |
| 谁需要关心？ | 沙箱运维团队、恶意软件分析师、威胁狩猎团队、任何需要检测反分析行为的安全人员 |
| 你的第一步防御 | 监控 `wmic cpu get manufacturer`、`wmic bios get serialnumber` 等硬件查询，检测长时间 Sleep 调用后的异常行为 |
| 如果只做一件事 | 对进程启动后执行硬件信息查询并紧接着长时间 Sleep 的行为模式立即告警，这是典型的沙箱检测特征 |
...
- [T1497.001 - 系统检查](./T1497/T1497.001-System-Checks.md) — 检查电脑的硬件配置，看是否像一台虚拟机。
- [T1497.002 - 基于用户活动的检查](./T1497/T1497.002-User-Activity-Checks.md) — 检查电脑前是不是真的有一个人在操作。
- [T1497.003 - 基于时间的规避](./T1497/T1497.003-Time-based-Evasion.md) — 检查时间是否走得太快或太慢，判断是否在仿真环境。
...
**用人话说：** 这条规则在监控有人用 WMI 查询 CPU 制造商信息。攻击者为什么要查这个？因为虚拟机的 CPU 制造商通常显示为 "VirtualBox"、"VMware" 或 "QEMU"，而真实物理机显示的是 "Intel" 或 "AMD"。恶意软件在启动时会检查这个信息——如果发现是虚拟机，就知道自己可能在安全分析师的沙箱里，就会停止运行或显示假的正常行为。正常情况下，IT 运维人员可能在做资产盘点时查这个，但普通用户或后台进程执行它就很可疑。如果你看到一个新启动的进程先查 CPU 信息，再查 BIOS 序列号，然后长时间 Sleep，这就是典型的沙箱检测行为链。
