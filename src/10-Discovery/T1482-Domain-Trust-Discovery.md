...
| 维度 | 你需要知道的 |
|------|-------------|
| 这是什么？ | 攻击者使用 `nltest /domain_trusts`、`Get-ADTrust`、`Get-ADForest` 枚举域间信任关系，识别跨域横向移动路径和可访问的其他域 |
| 为什么危险？ | 域信任是攻击者扩大攻击范围的关键跳板，通过信任关系可以从一个域直接访问另一个域，绕过单域的安全边界 |
| 谁需要关心？ | SOC分析师、AD管理员、蓝队威胁狩猎、任何需要检测跨域侦察行为的安全人员 |
| 你的第一步防御 | 监控 `nltest /domain_trusts` 从非域控主机执行的行为，审计 LDAP 查询 `trustedDomain` 对象的异常请求 |
| 如果只做一件事 | 对非 AD 管理人员执行域信任枚举立即告警，因为正常业务不需要了解域间信任架构 |
...
**用人话说：** 这条规则在监控有人执行 `nltest /domain_trusts` 查询域间信任关系。这个命令会返回当前域信任哪些其他域、信任方向（单向/双向）等信息。攻击者为什么要查这个？因为域信任就像"通行证"——如果 A 域信任 B 域，攻击者攻破 B 域后就能直接访问 A 域的资源，不需要再破解 A 域的密码。正常情况下，只有 AD 管理员在做域架构审计时才会用这个命令。如果你看到有人在非管理时段执行，或者从非域控的普通工作站上执行，那就是攻击者在"画地图"，寻找跨域攻击的路径。
