...
| 维度 | 你需要知道的 |
|------|-------------|
| 这是什么？ | 攻击者使用 `aws ec2 describe-instances`、`aws s3 ls`、`az vm list`、`gcloud compute instances list` 枚举云上所有计算、存储、网络、容器资源及其配置 |
| 为什么危险？ | 攻击者通过全面了解云环境拓扑，可以找到暴露公网的服务、配置错误的安全组、高价值数据库、可横向移动的 IAM 角色，从而制定精准的攻击路径 |
| 谁需要关心？ | 云安全团队、SOC分析师、IAM管理员、CSPM运维 |
| 你的第一步防御 | 监控 CloudTrail 中 `Describe*`、`List*` API 的异常批量调用；使用 CSPM 工具检测异常 API 枚举模式；实施最小权限 IAM 策略限制资源列举权限 |
| 如果只做一件事 | 对单个 IAM 用户/角色短时间内跨多个服务类型执行 `Describe*`/`List*` 调用立即告警——这是攻击者在"全面侦察"你的云环境 |
...
**用人话说：** 这条规则在监控有人批量列举云基础设施资源。攻击者拿到云凭证后第一件事就是"画地图"——用 `DescribeInstances` 看有哪些虚拟机、`ListBuckets` 看有哪些存储桶、`DescribeDBInstances` 看有哪些数据库。他们重点关注：暴露公网的实例（可能有未修补的漏洞）、配置为公开访问的 S3 存储桶（可能有敏感数据）、安全组规则宽松的实例（可以被直接访问）。正常情况下，运维人员在做资产盘点时可能偶尔调用这些 API，但单个用户短时间内跨多个服务类型批量调用就很可疑。CloudTrail 会记录所有 API 调用，配置异常检测规则可以自动告警。
