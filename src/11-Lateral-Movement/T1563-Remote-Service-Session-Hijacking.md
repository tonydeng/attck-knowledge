...
**通俗理解：** 偷走别人正在使用的SSH隧道——当管理员通过SSH连接到服务器时，攻击者劫持这个连接，就像在半路上接管了车辆的方向盘。

**详细说明：**
SSH支持多种会话复用和代理转发功能。攻击者可以在被入侵的系统上利用这些功能劫持SSH连接。常见的方法包括：

1. **SSH Agent Forwarding劫持**：当管理员使用SSH代理转发连接到跳板机时，攻击者可以从跳板机的SSH认证套接字中提取代理密钥，然后使用这些密钥连接到管理员信任的其他服务器
2. **SSH Control Master劫持**：OpenSSH支持连接共享（ControlMaster功能），允许多个SSH会话复用同一个TCP连接。攻击者可以通过控制套接字文件接管现有SSH连接
3. **SSH Config File篡改**：攻击者修改用户的SSH配置文件（`~/.ssh/config`）以劫持未来的SSH连接

这种方法对使用SSH进行系统管理的基础设施特别有效，尤其是在采用跳板机架构的环境中。

### T1563.002 - RDP Hijacking

**通俗理解：** 抢走管理员的远程桌面——当管理员通过RDP连接到服务器时，攻击者"霸占"这个远程桌面会话，获得管理员的身份和权限。

**详细说明：**
在Windows中，远程桌面服务（TermService）支持多个并发会话。管理员可以使用`tscon`命令（终端服务控制台）从一个会话切换到另一个会话。攻击者在获得SYSTEM权限后，可以使用`tscon`命令连接到任何现有的RDP会话，而不需要知道该会话用户的密码。具体的操作步骤：

1. 使用`query session`命令查看所有活跃的RDP会话
2. 获取SYSTEM权限（如使用PsExec或Mimikatz的`token::elevate`）
3. 执行`tscon <SessionID>`命令连接到目标会话
4. 攻击者立即获得该会话用户的所有权限

RDP劫持的风险极高，因为被劫持的会话通常属于具有高权限的管理员账户。
