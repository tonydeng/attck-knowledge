...
| 维度 | 你需要知道的 |
|------|-------------|
| 这是什么？ | 就像攻击者控制了公司的一个员工账号后，用这个账号给其他同事发钓鱼邮件——因为是"内部同事"发的，更容易让人上当。 |
| 为什么危险？ | 内部鱼叉式钓鱼的核心优势在于绕过传统安全防护。外部钓鱼邮件会经过邮件安全网关的过滤，但内部发件人的邮件通常不受同等程度的 |
| 谁需要关心？ | 安全监控团队、SOC分析师 |
| 你的第一步防御 | 监控内部邮箱的异常发件行为 |
| 如果只做一件事 | 传统的钓鱼攻击是从外部发送恶意邮件（如假装是银行的邮件） |
...
> 内部鱼叉式钓鱼是"从内打破"的横向移动——攻击者已经控制了公司内部的一个邮箱账号，然后用这个"自己人"的账号给其他同事发带有恶意附件的钓鱼邮件。因为邮件来自公司内部域名（比如admin@company.com）而且发件人是同事，收件人的警惕性会大大降低。攻击者还可能利用被盗的邮箱设置自动转发规则，监控高管的邮件往来并在合适的时机伪造回复。检测方法：关注一个内部账号在短时间内向大量收件人发送带附件的邮件、邮件内容包含诱导性链接或要求安装附件、以及异常发送时间（如凌晨3点发全公司邮件）。Exchange日志中的MessageTrackingLog是主要检查来源。
>
> **避坑指南**：未启用PowerShell脚本块日志；未监控异常会话令牌使用；只检查外部邮件，忽略内部钓鱼。
