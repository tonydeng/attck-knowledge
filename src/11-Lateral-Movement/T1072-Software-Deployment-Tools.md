...
| 维度 | 你需要知道的 |
|------|-------------|
| 这是什么？ | 就像攻击者收买了公司的快递分发中心——利用企业的软件分发系统（如SCCM），一次操作就能把恶意软件推送到成百上千台电脑。 |
| 为什么危险？ | 软件部署工具横向移动的独特之处在于其规模化和自动化特性。与攻击者逐台用RDP连接不同，利用部署工具可以在一次操作中覆盖成 |
| 谁需要关心？ | 安全监控团队、SOC分析师 |
| 你的第一步防御 | 监控SCCM管理控制台的异常访问 |
| 如果只做一件事 | 大企业通常有专门的软件分发系统，比如微软的SCCM（System Center Configurat |
...
> 企业常用的软件部署工具（如Microsoft SCCM、PDQ Deploy、Symantec Altiris）拥有在所有已注册机器上远程安装软件的权限——这正是攻击者梦寐以求的"一键横向"能力。攻击者攻陷了SCCM管理服务器后，可以创建一个部署包包含恶意软件，然后"推送"到全网所有机器上执行。这种横向移动方式非常隐蔽，因为软件分发本身是IT部门的正常操作，安全监控系统很难区分"管理员部署补丁"和"攻击者部署木马"。检测重点：非软件更新时段的大规模程序推送、不在更新列表中的软件包被分发、SCCM服务器上新增了非授权管理员账号、以及Ccmsetup.exe或类似分发代理的异常行为。
>
