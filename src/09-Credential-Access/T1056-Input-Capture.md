...
**通俗理解：** 造一个和真的一模一样的假登录页面，骗你输入密码。

**详细说明：** 攻击者创建与合法网站（如银行、邮箱、企业VPN）外观完全一致的钓鱼页面。当受害者输入凭证时，页面将数据发送到攻击者的服务器，同时重定向到真实网站以降低怀疑。

### T1056.002 - GUI输入捕获

**通俗理解：** 在你电脑里装一个"录音机"，记录你敲的每一个字母。

**详细说明：** 使用SetWindowsHookEx API安装全局键盘钩子，或在Linux中使用`LD_PRELOAD`注入记录键盘输入。高级版本还能配合屏幕截图，捕获虚拟键盘的点击位置。

### T1056.004 - 凭证API钩子

**通俗理解：** 在系统的"密码输入接口"上装一个分叉管，你输入的密码系统收到一份，黑客也收到一份。

**详细说明：** 通过API Hook技术拦截Windows认证函数（如CredUnPackAuthenticationBuffer、LsaCallAuthenticationPackage），在进程调用这些函数时捕获传递的密码参数。
