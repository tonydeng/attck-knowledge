...
```mermaid
graph TD
    A["获取管理员权限"] --> B["枚举Windows凭据保管库"]
    B --> C["使用VaultCmd或mimikatz提取"]
    C --> D["解密DPAPI保护的凭证"]
    D --> E["获取保存的网站和应用密码"]
...
### 案例1：APT组织使用Windows Credential Manager进行攻击准备

- **时间**: 2023-2024年
- **目标**: 多行业目标组织
- **攻击组织**: 多个APT组织
- **手法**: 攻击者在攻击初期大量使用Windows Credential Manager技术收集目标信息，为后续定向攻击做准备
- **影响**: 攻击成功率显著提高，防御者难以及时发现侦察行为
- **参考链接**: [MITRE ATT&CK - T1555.004](https://attack.mitre.org/techniques/T1555/004/)

### 案例2：红队演练中的Windows Credential Manager应用

- **时间**: 2024-2025年
- **目标**: 授权测试的企业客户
- **攻击组织**: 红队团队
- **手法**: 在授权的红队演练中，Windows Credential Manager被用于模拟真实攻击者的信息收集行为，测试企业安全监控体系能否及时发现侦察活动
...
| 工具名称 | 用途 | 平台 |
|---------|------|------|
| 专用收集工具 | Windows Credential Manager相关操作 | 全平台 |
