...
**通俗理解：** 伪造了一张"企业门禁卡"，可以进公司的任何系统——ERP、企业邮箱、云服务。

**详细说明：**
SAML（安全断言标记语言）是一种基于XML的开放标准，用于在企业身份提供商（IdP）和服务提供商（SP）之间交换认证数据。当用户登录企业邮箱时，流程如下：用户访问邮箱（SP）→ 邮箱将用户重定向到Azure AD（IdP）→ Azure AD验证身份后签发一个SAML断言 → 用户带着断言访问邮箱 → 邮箱验证断言签名后允许登录。如果攻击者窃取了IdP的签名私钥（如AD FS的Token-Signing Certificate），就可以跳过"验证身份"这一步，直接签发SAML断言。Golden SAML就是这种攻击的经典实现。

### T1606.002 - OAuth Tokens

**通俗理解：** 伪造了一把"API钥匙"，可以调用任何云服务的接口。

**详细说明：**
OAuth 2.0是一种授权框架，允许第三方应用在不获取用户密码的情况下获得对受保护资源的有限访问权限。OAuth令牌通常以JWT格式实现。攻击者伪造OAuth令牌的方法包括：JWT算法混淆（将RS256改为HS256，使用已知的公钥作为HMAC密钥签名）、窃取客户端凭据（client_id和client_secret直接获取令牌）、授权码拦截（在授权码流程中截获或推断授权码）、以及OBO（On-Behalf-Of）令牌交换滥用。
