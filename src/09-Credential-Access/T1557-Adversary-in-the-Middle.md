...
**通俗理解：** 当你的电脑找不到某台机器时，会大喊"谁是XXX？"，攻击者抢先回答"是我！"骗你把密码发过来。

**详细说明：**
Windows系统在无法通过DNS解析主机名时，会使用LLMNR（链路本地多播名称解析，UDP 5355）和NBT-NS（NetBIOS名称服务，UDP 137）进行广播查询。攻击者使用Responder或Inveigh等工具监听这些广播，并伪造应答声称自己是目标主机。受害者随后向攻击者发送NTLM认证请求。在SMB Relay变体中，攻击者不直接破解捕获的Net-NTLMv2哈希，而是将认证请求中继到目标服务器（如域控制器），以受害者身份完成认证。

### T1557.002 - ARP Cache Poisoning

**通俗理解：** 在网络中散播假的地图，让所有人把寄给别人的信都送到你这里来。

**详细说明：**
ARP（地址解析协议）用于在局域网中将IP地址解析为MAC地址（物理地址）。攻击者向目标网络发送伪造的ARP应答包，声称某个IP地址对应的MAC地址是攻击者的MAC。受害者的ARP缓存被毒化后，所有发往该IP的流量都被重定向到攻击者。攻击者启用IP转发，将流量正确路由到原始目标以保持通信不中断，同时中间拦截所有数据。

### T1557.003 - DHCP Spoofing

**通俗理解：** 攻击者冒充公司的网络管理员，给新接入的设备分配错误的网络设置。

**详细说明：**
DHCP（动态主机配置协议）负责给网络中的设备自动分配IP地址、网关和DNS等配置。攻击者在网络上部署恶意的DHCP服务器，比合法DHCP更快响应客户端的请求（竞争），或耗尽合法DHCP的地址池后分配恶意配置（饥饿攻击）。伪造的配置将默认网关指向攻击者，使所有出站流量经过攻击者的设备。

### T1557.004 - Mutual Authentication Impairment

**通俗理解：** 破坏服务器和客户端之间的相互身份验证，让攻击者可以冒充服务器。

**详细说明：**
现代安全协议（TLS、Kerberos、HTTPS）设计了双向认证来防止中间人攻击——客户端验证服务器的证书，服务器也验证客户端的身份。攻击者通过以下方式破坏双向认证：安装自签名的根证书到受信任的根证书颁发机构存储、修改系统注册表禁用TLS证书验证、使用SSLsplit动态生成和注入证书、或强制使用弱密码套件。
