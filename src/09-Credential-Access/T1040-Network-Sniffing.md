...
| 维度 | 你需要知道的 |
|------|-------------|
| 这是什么？ | 在网络上传输时截获密码、令牌等认证信息 |
| 为什么危险？ | 网络嗅探是最经典的凭证获取方式，明文传输的密码可以直接被捕获，即使加密也可能被破解 |
| 谁需要关心？ | 网络管理员、SOC分析师 |
| 你的第一步防御 | 使用加密协议（HTTPS、SSH）替代明文协议，部署网络入侵检测系统 |
| 如果只做一件事 | 监控网络接口的混杂模式启用（Sysmon事件ID），任何非抓包工具启用混杂模式立即告警 |
...
**用人话说：** 这条规则在监控网络接口是否被设置为混杂模式。混杂模式是网络嗅探的基础，它让网卡能接收所有网络流量而不仅仅是发给自己的。正常情况下只有网络监控工具（如Wireshark）会使用混杂模式。如果有陌生进程启用混杂模式，那就是攻击者在网络中'偷听'传输的密码和敏感数据。
