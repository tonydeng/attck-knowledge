...
```mermaid
graph TD
    A["获取Linux root权限"] --> B["读取/etc/passwd获取用户名列表"]
    B --> C["读取/etc/shadow获取密码哈希"]
    C --> D["组合用户名与哈希对"]
    D --> E["使用John/Hashcat离线破解密码"]
...
- **目标**: 多个行业组织
- **攻击组织**: 多个APT组织
- **手法**: 攻击者在入侵过程中使用/etc/passwd和/etc/shadow技术，展示了该子技术的典型应用场景和攻击效果
...
- **目标**: 安全研究测试环境
- **攻击组织**: 红队/安全研究人员
- **手法**: 在授权测试中模拟/etc/passwd和/etc/shadow攻击，验证防御体系的检测和响应能力
