...
| 维度 | 你需要知道的 |
|------|-------------|
| 这是什么？ | 一个一个试密码，总能试对——暴力破解是最直接的密码攻击方式 |
| 为什么危险？ | 只要密码不够强，暴力破解迟早能试出来，而且自动化工具可以每秒尝试数千次 |
| 谁需要关心？ | SOC分析师、安全运营团队 |
| 你的第一步防御 | 部署账户锁定策略，监控异常的登录失败模式 |
| 如果只做一件事 | 监控Windows事件ID 4625（登录失败），短时间内大量失败登录尝试是暴力破解的典型特征 |
...
- [T1110.001 - 密码猜测](./T1110/T1110.001-Password-Guessing.md) — 根据目标的个人信息推测密码。
- [T1110.003 - 密码喷射](./T1110/T1110.003-Password-Spraying.md) — 用一个常见密码试几千个账号，不会触发单账号锁定。
- [T1110.004 - 凭证填充](./T1110/T1110.004-Credential-Stuffing.md) — 用从其他网站偷来的用户名密码组合试登录。
...
**用人话说：** 这条规则在监控是否有大量的登录失败事件。暴力破解是攻击者用自动化工具不断尝试密码。正常情况下用户偶尔输错密码是正常的，但如果同一个账户或同一组IP在短时间内出现大量登录失败，那很可能是在进行暴力破解攻击。
