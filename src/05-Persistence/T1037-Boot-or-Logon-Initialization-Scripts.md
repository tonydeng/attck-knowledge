...
| 维度 | 你需要知道的 |
|------|-------------|
| 这是什么？ | 攻击者修改Windows登录脚本、Linux .bashrc/.profile、macOS loginhook等系统初始化脚本，每次用户登录时自动执行恶意代码 |
| 为什么危险？ | 利用系统合法功能实现持久化，重启后依然有效，极难被普通用户发现 |
| 谁需要关心？ | 系统管理员、安全运营团队、SOC分析师 |
| 你的第一步防御 | 审计组策略登录脚本、shell配置文件（.bashrc/.profile）的修改，建立正常行为基线 |
| 如果只做一件事 | 对SYSVOL目录下的脚本文件建立白名单监控，任何新增或修改都需要安全团队确认 |
