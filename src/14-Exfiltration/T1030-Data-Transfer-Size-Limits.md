...
| 维度 | 你需要知道的 |
|------|-------------|
| 这是什么？ | 数据大小限制（T1030）是攻击者用来破坏目标系统或数据的技术 |
...
**用人话说：** 攻击者把大文件切成很多小块，每个小块单独传输，目的是绕过网络出口的文件大小检测——就像把一整箱珠宝分装到多个小口袋里分批带出安检门。正常业务传输中，文件大小是自然分布的（有大有小、参差不齐），而分片传输的特征是多份文件大小惊人一致（比如全是49.9MB）。如果看到同一台机器在短时间内向同一外部IP发送大量大小几乎完全相同的文件，很可能是攻击者在用分片方式偷数据。
