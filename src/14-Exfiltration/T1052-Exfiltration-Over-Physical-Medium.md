...
| 维度 | 你需要知道的 |
|------|-------------|
| 这是什么？ | 通过物理介质渗漏（T1052）是攻击者用来破坏目标系统或数据的技术 |
...
- [T1052.001 - USB设备](./T1052/T1052.001-USB-USB.md) — 把文件拷到U盘上揣兜里带走。
- [T1052.002 - 空气间隙](./T1052/T1052.002-Air-Gap-Exfiltration.md) — 从完全没有网线的电脑上把数据弄出来。
...
**用人话说：** 攻击者不用网络传输数据，而是把文件拷贝到U盘、外置硬盘等物理设备上，人带着设备走出大楼——网络监控再严密也拦不住物理搬运。与网络渗漏不同，物理介质渗漏完全不产生网络出口流量，防火墙、DLP、IDS全部失效。如果检测到某台电脑在短时间内向USB可移动驱动器拷贝大量文件（特别是文档、数据库等敏感类型文件），或者出现非授权USB存储设备的接入事件，需要高度警惕物理数据窃取行为。
