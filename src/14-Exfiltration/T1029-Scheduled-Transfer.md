...
| 维度 | 你需要知道的 |
|------|-------------|
| 这是什么？ | 定时传输（T1029）是攻击者用来破坏目标系统或数据的技术 |
...
**用人话说：** 攻击者设定好时间（比如凌晨2点），让恶意程序自动把窃取的数据打包发出去，就像小偷提前踩点好保安换岗的时间再动手。与全天候实时传输不同，定时传输精准选在安全监控最薄弱的时间窗口（深夜、周末、节假日）执行。如果发现某台服务器在非工作时间规律性地向外传输数据包，且传输模式与业务备份窗口不匹配，就要怀疑是定时传输渗漏。
