...
| 维度 | 你需要知道的 |
|------|-------------|
| 这是什么？ | 传输数据到云账户（T1537）是攻击者用来破坏目标系统或数据的技术 |
...
**用人话说：** 攻击者不是把数据从内网下载到外部，而是利用窃取的云凭证直接在云平台内部把数据从一个账户复制到另一个账户——数据全程在云服务商的骨干网内传输，根本不经过受害者的网络出口，传统网络监控完全看不到。如果发现云环境中有非预期的跨账户数据复制操作（如AWS的CopyObject API、Azure的AzCopy跨租户传输），或者IAM角色突然新增了指向未知外部账户的信任策略，需要立即排查是否发生了云账户间的数据盗取行为。
