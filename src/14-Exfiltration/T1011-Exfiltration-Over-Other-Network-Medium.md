...
| 维度 | 你需要知道的 |
|------|-------------|
| 这是什么？ | 通过其他网络介质渗漏（T1011）是攻击者用来破坏目标系统或数据的技术 |
...
**用人话说：** 攻击者不走公司的标准网络出口（宽带/Wi-Fi），而是用蓝牙、USB无线网卡、4G上网卡等偏门通道把数据传出去——就像小偷发现正门有监控，从通风管道爬走。这类攻击不产生标准的网络出口流量，防火墙和IDS根本看不到。如果发现服务器突然启用了蓝牙或无线网卡接口，或者系统上出现了非预期的网络设备接入事件，需要警惕攻击者正在架设替代网络介质通道。
