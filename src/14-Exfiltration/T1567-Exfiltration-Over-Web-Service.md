...
| 维度 | 你需要知道的 |
|------|-------------|
| 这是什么？ | 攻击者利用百度网盘、GitHub、Pastebin等正规网络服务，把窃取的数据伪装成正常用户行为上传到云端，再从外部下载 |
| 为什么危险？ | 流量流向的是"受信任"的域名（如 microsoft.com、github.com），传统黑名单防火墙和DLP系统无法区分正常使用和恶意上传 |
| 谁需要关心？ | 安全运维团队、云安全工程师、DLP管理员 |
| 你的第一步防御 | 对云服务API调用实施严格的频率和流量基线监控，限制非授权的云存储应用 |
| 如果只做一件事 | 启用云访问安全代理（CASB）或DLP规则，监控异常大文件上传到网盘/代码仓库的行为 |
...
- [T1567.001 - 云存储](./T1567/T1567.001-Cloud-Storage.md) — 就是把你偷的文件传到百度网盘上，然后从另一个电脑下载。
- [T1567.002 - 代码仓库](./T1567/T1567.002-Code-Repository.md) — 把偷来的数据当作代码提交到GitHub仓库里。
- [T1567.003 - 文本分享网站](./T1567/T1567.003-Paste-Site.md) — 用Pastebin这类网站贴代码的功能，把数据贴上去再复制走。
- [T1567.004 - Webhook回调](./T1567/T1567.004-Webhook-Webhook.md) — 用企业微信或钉钉的机器人接口，把数据当消息发出去。
...
**用人话说：** 攻击者利用合法的云服务（如网盘、GitHub、Pastebin）来传输窃取的数据，因为流量流向的是受信任域名（如microsoft.com、github.com），防火墙和DLP通常放行不检查。就像小偷用你的快递账号把赃物寄到自己家——用正规快递公司送货，门卫不会怀疑。如果发现内网机器突然向云存储服务上传大量非业务相关的文件，或者在非工作时间有异常的API调用（如rclone同步、curl上传），需要警惕攻击者正在利用Web服务作为数据外泄通道。
