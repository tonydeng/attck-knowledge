...
| 维度 | 你需要知道的 |
|------|-------------|
| 这是什么？ | 攻击者不用常规的HTTP/HTTPS通道，而是通过DNS查询、ICMP(Ping)包、电子邮件等"非主流"协议把偷到的数据运出去 |
| 为什么危险？ | DNS、ICMP、SMTP等协议通常是防火墙放行的白名单流量，安全设备很少深度检查其内容，攻击者可以神不知鬼不觉地把数据传出网络 |
| 谁需要关心？ | 网络安全工程师、SOC分析师、威胁狩猎团队 |
| 你的第一步防御 | 对DNS查询做异常检测（如超过正常长度的查询名、极高频率的DNS请求），限制ICMP数据包大小 |
| 如果只做一件事 | 部署DNS隧道检测规则，监控超长域名查询和异常的TXT记录请求 |
...
- [T1048.002 - 对称加密渗出](./T1048/T1048.002-Symmetric-Exfiltration.md) — 把数据装进一个密码箱（AES加密），虽然箱子被看到了但密码只有双方知道。
- [T1048.001 - 非对称加密渗出](./T1048/T1048.001-Asymmetric-Exfiltration.md) — 别人把一个公开的邮箱地址给你，你把信投进去，只有有钥匙的人才能打开邮箱取信。
- [T1048.003 - 未加密渗出](./T1048/T1048.003-Plaintext-Exfiltration.md) — 直接用明信片寄出秘密信息，快但容易被别人看到。
...
**用人话说：** 攻击者不用HTTP/HTTPS这类常规协议，而是把数据藏在DNS查询、ICMP（Ping）包或电子邮件等非预期协议里发出去。这些协议通常是防火墙默认放行的白名单流量，很少被深度检查——就像把赃物藏在垃圾车里运出小区。如果发现内网出现超长域名的DNS查询（子域名超过30个字符）、异常的TXT记录请求、非标准端口上的协议使用（如53端口上的非DNS流量），或者ICMP数据包大小远超正常值（正常Ping只有几十字节），很可能有人在用替代协议偷数据。
